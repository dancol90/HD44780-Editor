...
void mousePressed() {
  ...
  if(checkButtonClick(55, 20)) {
    ...
    selectOutput("Export font...", "export");
  else if(checkButtonClick(170))
    // Save button clicked.
    selectOutput("Save font", "saveFile");
  else if(checkButtonClick(145))
    // Open button clicked.
    selectInput("Open font", "openFile");
...
void mouseReleased() {
  ...
  if(current_dragging_char != -1) {
    ...
    if(mouseInRect(lcd_pos, lcd_size)) {
      // ...set that lcd char.
...
void saveFile(File selection) {
  ...
  if (selection != null) {
    ...
    saveBytes(selection.getAbsolutePath(), data);
  }
}
