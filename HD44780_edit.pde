...
static final int[] lcd_char_dim = {5 * lcd_scale + 1, 7 * lcd_scale + 1};  // CALCULATED. Width & height of a single lcd char 
static final int[] lcd_size = {20 * lcd_char_dim[0], 4 * lcd_char_dim[1]}; // CALCULATED. Width & height of lcd preview.

static final int custom_chars_pos[] = {10, 130}; // Position of custom chars thumbnails
static final int custom_chars_scale  = 5;        // Scale of custom chars thumbnail
static final int[] custom_char_dim   = {5 * custom_chars_scale + 1, 7 * custom_chars_scale + 1}; // CALCULATED. Width & height of a single thumbnail
static final int[] custom_chars_size = {4 * custom_char_dim[0], 2 * custom_char_dim[1]};         // CALCULATED. Width & height of all thumbnail

static final int edit_char_pos[] = {135, 130}; // Position of editing area
...
void draw() {
  ...
  rect(width - 10 - 83, 125, 100, 145);
  noStroke();
  
  // Draw actual buttons.
  drawButton(width - 10 - 75, 130, 75, 20, "Open", true);
  drawButton(width - 10 - 75, 155, 75, 20, "Save", true);
  drawButton(width - 10 - 75, 180, 75, 20, "Export");
  
  // Here caption is different if we have alredy clicked once the button.
  drawButton(width - 10 - 75, 245, 75, 20, sure_about_reset ? "Are you sure?" : "Reset all");
...
void mousePressed() {
  ...
  if(checkButtonClick(180)) {
    // Export button clicked.
    do_export = true;
  } else if(checkButtonClick(245)) {
...
void export() {  
  ...
  if (savePath != null) {
    ...
    output.println("const uint8_t custom_chars[8][7] = {");
...
boolean mouseInRect(int[] pos, int charScale) {
  return mouseInRect(pos[0], pos[1], 5 * charScale, 7 * charScale);
}
